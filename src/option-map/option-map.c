...
static void str_arr_destroy(char **str_arr);
...
struct OptionMapOption *option_map_get_option_by_name(struct OptionMap *option_map, const char *option_name)
{
	for (size_t index = 0; index < option_map->option_count; ++index) {
		struct OptionMapOption *option = &option_map->options[index];
...
struct OptionMapOption *option_map_get_option_by_alias(struct OptionMap *option_map, const char *option_alias)
{
	for (size_t index = 0; index < option_map->option_count; ++index) {
		struct OptionMapOption *option = &option_map->options[index];
