...
struct SCGBuffer *scg_pixel_buffer_create(uint16_t width, uint16_t height)
{
...
void scg_pixel_buffer_set(struct SCGBuffer *buffer, uint16_t col, uint16_t row, enum SCGColorCode color)
{
	scg_buffer_set_bg_color(buffer, col * 2, row, color);
	scg_buffer_set_bg_color(buffer, col * 2 + 1, row, color);
}

enum SCGColorCode scg_pixel_buffer_get(struct SCGBuffer *buffer, uint16_t col, uint16_t row)
{
...
uint16_t scg_pixel_buffer_get_width(struct SCGBuffer *buffer)
{
	return buffer->width / 2;
}

uint16_t scg_pixel_buffer_get_height(struct SCGBuffer *buffer)
{
